...
for file in *.avi; do
  ...
  Zbiory treningowe dla metody wykorzystującej las drzew losowych generowane są na podstawie przekształceń afinicznych oraz szumów wprowadzonych do przygotowanej pierwszej klatki anmiacji (to przekształcenie zostało omówione w~sekcji \ref{Subsection_AdjustFirstFrame}).
  ...
  \begin{itemize}
    ...
    \item Pochylenie na osi X z~zakresu $<0, 0.41>$ co 0.1.
    \item Pochylenie na osi Y z~zakresu $<0, 0.41>$ co 0.1.
    \item Przeskalowanie na osi X z~zakresu $<0.2, 1.1>$ co 0.2.
    \item Szum jednostajny (ang. \textit{uniform noise}).
  \end{itemize}
