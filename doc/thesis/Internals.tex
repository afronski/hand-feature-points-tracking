...
./bin/tracking assets/Person_A_C.avi "Random Forest Tracker" 30 100
  ...
  \section{Interfejs użytkownika do prezentacja wykresów z~zebranych danych}

    Zupełnie odrębnym widokiem zaprojektowanym i~zaimplementowanym w~aplikacji jest prezentacja wykresów z~zebranych danych pomiarowych. Został on zbudowany w~oparciu o~technologie szczegółowo omówione w~rozdziale \ref{Subsection_PomocniczeTechnologie}. Na rysunku \ref{fig:Charts} został zaprezentowany przykładowy widok.

    \newpage
    \begin{figure}[!ht]
      \centering
      \includegraphics[width=14cm]{Charts}
      \caption[Interfejs graficzny prezentujący wykresy z~zebranych danych]{Interfejs graficzny prezentujący wykresy z~zebranych danych}
      \label{fig:Charts}
    \end{figure}

    Użytkownik za pomocą pola wyboru umieszczonego w~górnej części interfejsu użytkownika ma możliwość wykonania odpowiedniego typu wykresu. Po zaznaczeniu odpowiedniej opcji (oraz krótkiej chwili, ponieważ generacja oraz rysowanie wykresów jest procesem kosztownym obliczeniowo) w~oknie przeglądarki pojawią się wygenerowane rezultaty.

    Prezentacja wszystkich wykresów danego typu na jednej stronie ułatwia porównywanie pomiędzy poszczególnymi wynikami. Stworzone wykresy są interaktywne - możliwe jest wyłączenie poszczególnych serii oraz sprawdzenie dokładnej wartości jaka znajduje się w~danym punkcie.

    Warto nadmienić, że rezultaty zamieszczone w~pracy w~rozdziale \ref{Chapter_AnalizaRezultatow} zostały stworzone właśnie za pomocą tego widoku.